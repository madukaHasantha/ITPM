...
router.route("/announcementAdd").post(async (req, res) => {
  const { announcementID, catagory, toWhome, from, message, date, time } =
    req.body;
  ...
  if (announcement == null) {
    newAnnouncement
      ...
      .catch((err) => {
        ...
        return res
          .status(400)
          .send({ status: "Erro with published a announcement!" });
      });
  } else {
...
router.route("/updateAnnouncement").put(async (req, res) => {
  try {
    ...
    if (announcement != null) {
      ...
      return res.status(400).json({
        message:
          "There is no any announcement in this announcementID to update!",
      });
    }
  } catch (e) {
